...
class InfinoSDK:
    ...
    def create_dataset_with_mapping(
        self, dataset: str, mapping: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Create a dataset with explicit field mappings"""
        url = f"{self.endpoint}/{dataset}"
        response = self.request("PUT", url, body=json.dumps(mapping))
        return response
    ...
    def count(self, dataset: str) -> Dict[str, Any]:
        """Get the document count for a dataset"""
        url = f"{self.endpoint}/{dataset}/_count"
        response = self.request("GET", url)
        return response

    def flush(self) -> Dict[str, Any]:
        """Flush all datasets (force commit uncommitted segments)"""
    ...
    async def generate_notebook_report(
        ...
        client_id = f"notebook_{notebook_id}"
        ws = await self.websocket_connect(
            "/fino/ws",
            {"x-infino-ws-thread": notebook_id, "x-infino-ws-client": client_id},
        )
        ...
        if analysis_cache:
            msg["analysis_cache"] = analysis_cache
            ...
            async for raw in ws:
                ...
                if msg_type == "error":
                    ...
                    raise InfinoError(
                        ...
                        str(data.get("error_message", "Report generation failed")),
                    )
    ...
    async def _query_via_fino_ws(
        ...
        try:
            async for raw in ws:
                ...
                if role and content and isinstance(content, dict):
                    msg_type = content.get("type", "")

                    if msg_type in ("message", "result"):
                        await ws.close()
                        return {
                            "answer": content.get("summary")
                            or content.get("message")
                            or content.get("text", ""),
                            "data": content.get("data", {}),
                            "querydsl": content.get("querydsl"),
                            "sql": content.get("sql"),
                    ...
                    if msg_type == "error":
                        ...
                        raise InfinoError(
                            ...
                            or content.get("message", "Query failed"),
                        )
        except asyncio.TimeoutError:
            await ws.close()
            raise InfinoError(InfinoError.Type.TIMEOUT, "Query timed out")
    ...
    def web_search(
        self, connection_id: str, query: str, max_results: int = 10
    ) -> Dict[str, Any]:
        """Perform a web search through a web-type connector"""
        url = f"{self.endpoint}/source/{connection_id}/web"
        payload = {"search_query": query, "max_results": max_results}
        response = self.request("POST", url, body=json.dumps(payload))
        return response

    def get_all_datasets(self, connection_id: str) -> List[Dict[str, Any]]:
        """Get all datasets (tables/indices) available in a remote source"""
        url = f"{self.endpoint}/source/{connection_id}/datasets"
        response = self.request("GET", url)
        if isinstance(response, list):
            return response
        if isinstance(response, dict):
    ...
    def upload_file(
        ...
        dataset: str,
        file_path: str,
        format: Optional[str] = "auto",  # noqa: A002
    ...
    def update_user_password(self, username: str, new_password: str) -> Dict[str, Any]:
        ...
        response = self.request("PATCH", url, body=json.dumps(payload))
        return response

    def get_user_account_info(self) -> Dict[str, Any]:
        """Get the current account info for the authenticated user"""
        url = f"{self.endpoint}/account"
        response = self.request("GET", url)
        return response

    def get_user_auth_info(self) -> Dict[str, Any]:
        """Get auth info (whoami) for the current authenticated user"""
        url = f"{self.endpoint}/user/auth"
        response = self.request("GET", url)
        return response

    def generate_user_token(
        self, username: str, duration_days: int = 30
    ) -> Dict[str, Any]:
        """Generate a bearer token for a user"""
        url = f"{self.endpoint}/user/{username}/token"
