...
async def send_query_and_receive_response():
    ...
    try:
        ...
        async with sdk.websocket_connect(
            "/_conversation/ws", headers=headers
        ) as websocket:
            print("✓ Connected!")
                    ...
                    async for message in websocket:
                        ...
                        if msg_type == "update":
                            ...
                            if "results" in content:
                                print(
                                    f"Results: {json.dumps(content['results'], indent=2)}"
                                )
    ...
    try:
        ...
        async with sdk.websocket_connect(
            ...
            for i, query_text in enumerate(queries, 1):
                ...
                # Send query
                query = {"content": {"user_query": query_text}}
...
async def handle_websocket_errors():
    ...
    try:
        ...
        async with sdk.websocket_connect(
            ...
            try:
                # Send query
                query = {"content": {"user_query": "Test query"}}
                await websocket.send(json.dumps(query))
                print("✓ Query sent")
                ...
                async with asyncio.timeout(30):
                    ...
                    async for message in websocket:
                        ...
                        if content.get("type") == "message":
                            print(
                                f"✓ Received final response after {message_count} message(s)"
                            )
                            break
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            async with sdk.websocket_connect(
                ...
                # Send a query
                query = {
                    "content": {"user_query": "What is the current system status?"}
                }
                await websocket.send(json.dumps(query))
...
async def streaming_data_visualization():
    ...
    try:
        ...
        async with sdk.websocket_connect(
            ...
            query = {
                "content": {"user_query": "Analyze error patterns in the last 24 hours"}
            }
            await websocket.send(json.dumps(query))
